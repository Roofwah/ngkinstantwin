...
export default function FulfilPrize() {
  ...
  async function handleFulfil() {
    ...
    setError('');
    try {
      const next = await completeFulfilment(token);
      setData(next);
    } catch (err) {
  ...
  return (
    <HokaCampaignShell artSrc="/campaigns/hoka/slide5.jpg">
      ...
      {data && (
        <>
          ...
          {!fulfilled && (
            <button className="hoka-cta" type="button" onClick={handleFulfil} disabled={saving}>
